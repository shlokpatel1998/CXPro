...
describe('getMembersForProject', () => {
    ...
    mockFrom.mockImplementation((table: string) => {
          ...
          select: vi.fn().mockImplementation(() => ({
            eq: vi.fn().mockImplementation((column: string, value: any) => {
              if (column === 'project_id') {
                const data = mockData.participations
                  .filter(p => p.project_id === value)
          ...
          select: vi.fn().mockImplementation(() => ({
            in: vi.fn().mockImplementation((column: string, values: any[]) => ({
              eq: vi.fn().mockImplementation((column2: string, value2: any) => {
                const data = mockData.memberships
                  .filter(m => values.includes(m.user_id) && m.org_id === value2)
      ...
      if (table === 'discipline_scopes') {
        return {
          select: vi.fn().mockImplementation(() => ({
            eq: vi.fn().mockImplementation((column: string, value: any) => {
              const data = mockData.discipline_scopes
                .filter(ds => ds.project_id === value)
            ...
            in: vi.fn().mockImplementation((column: string, values: any[]) => ({
              in: vi.fn().mockImplementation((column2: string, values2: any[]) => {
                const data = mockData.assignments
                  .filter(a => values.includes(a.user_id) && values2.includes(a.discipline_scope_id))
                  .map(a => ({ user_id: a.user_id, discipline_scope_id: a.discipline_scope_id }))
                return { data, error: null }
              })
            }))
          }))
  ...
  it('handles members without discipline assignments', async () => {
    mockData.projects = [{ id: 'project-1', org_id: 'org-1' }]
    mockData.users = [{ id: 'user-1', email: 'alice@example.com', full_name: 'Alice Smith' }]
    mockData.memberships = [{ user_id: 'user-1', org_id: 'org-1', role: 'OCA' }]
    mockData.participations = [{ user_id: 'user-1', project_id: 'project-1', org_id: 'org-1' }]
    mockData.assignments = []
...
describe('getPendingInvitesForProject', () => {
      ...
      if (table === 'pending_invitations') {
        ...
        const queryBuilder = {
          ...
          gt: vi.fn().mockImplementation((column: string, value: any) => {
            ...
            return { data: invitationsData, error: null }
          })
        }
  ...
  it('returns unexpired unaccepted invitations for the project', async () => {
    ...
    mockData.users = [{ id: 'user-admin', email: 'admin@example.com' }]
    mockData.discipline_scopes = [
      { id: 'ds-1', project_id: 'project-1', name: 'Mechanical' },
      { id: 'ds-2', project_id: 'project-1', name: 'Electrical' }
    ]
              ...
              if (column === 'user_id') {
                ...
                return {
                  eq: vi.fn().mockImplementation(() => {
                    ...
                    return { data: userAssignments, error: null }
                  })
                }
              }
              return { data: [], error: null }
          ...
          select: vi.fn().mockImplementation(() => ({
            eq: vi.fn().mockImplementation((column: string, value: any) => {
              if (column === 'project_id') {
                ...
                return { data: projectScopes, error: null }
              }
              return { data: [], error: null }
            })
          }))
...
describe('getCurrentUserRole', () => {
  ...
  it('returns cx_engineer for a user with cx_engineer role', async () => {
    mockData.projects = [{ id: 'project-1', org_id: 'org-1' }]
    mockData.memberships = [{ user_id: 'user-2', org_id: 'org-1', role: 'cx_engineer' }]
  ...
  it('returns null when user has no membership in the project org', async () => {
    mockData.projects = [{ id: 'project-1', org_id: 'org-1' }]
    mockData.memberships = [{ user_id: 'user-3', org_id: 'org-2', role: 'OCA' }]
